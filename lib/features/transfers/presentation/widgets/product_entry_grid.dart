...
class ProductEntryGridState extends State<ProductEntryGrid> {
  ...
  List<TransferLineDraft> _validatedLines() {
    return _rows
        .where((r) => r.isResolved && r.productId != null)
        .map(
          (r) => TransferLineDraft(
  ...
  Future<void> _searchRow(
    ...
    row.quantity = double.tryParse(row.qtyController.text.trim()) ?? 1;
  ...
  /// Tras Enter: agrega una fila nueva si hace falta y mueve el foco al codigo siguiente.
      ...
      children: [
        ...
        Padding(
          ...
          child: TextField(
            ...
            onChanged: (v) {
              row.quantity = double.tryParse(v.trim()) ?? 0;
              if (row.isResolved) {
                _notifyChange();
                _validateStock(index);
              }
