...
class _CreateTransferScreenState extends ConsumerState<CreateTransferScreen> {
  ...
  TextEditingController? _originAutocompleteController;
  ...
  void _selectPrimaryOrigin(String value, List<String> validKeys) {
    final trimmed = value.trim();
    if (trimmed.isEmpty) {
      setState(() => _primaryOriginKey = null);
      return;
    }
    String? match;
    ...
    setState(() => _primaryOriginKey = match);
    final c = _originAutocompleteController;
    if (c != null && c.text != match) {
  ...
  void _openProcessPanel() {
    ...
    if (_gridKey.currentState?.hasZeroQuantityResolvedLines == true) {
      AppSnackbar.show(
        context,
        message:
            'Hay productos con cantidad 0. Indique cuantas unidades transferir.',
        isError: true,
      );
      return;
    }
                                    ...
                                    children: [
                                      ...
                                      Autocomplete<String>(
                                        ...
                                        fieldViewBuilder: (
                                          ...
                                          _originAutocompleteController =
                                              controller;
                                          final saved = _primaryOriginKey;
                                          if (saved != null &&
                                              controller.text != saved) {
                                            WidgetsBinding.instance
                                                .addPostFrameCallback((_) {
                                              if (!mounted) return;
                                              if (_primaryOriginKey == saved &&
                                                  controller.text != saved) {
                                                controller.value =
                                                    TextEditingValue(
                                                  text: saved,
