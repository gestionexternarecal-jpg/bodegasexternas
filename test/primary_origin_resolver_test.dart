...
void main() {
  group('PrimaryOriginResolver', () {
    ...
    test('uniqueOriginKeys omits excluded warehouse names', () {
      const revision = StockLocation(
        id: 10,
        name: 'Bodega en Revisión',
        completeName: 'PHSA/Bodega en Revisión',
      );
      const falla = StockLocation(
        id: 11,
        name: 'Productos con Falla',
        completeName: 'PHSA/Productos con Falla',
      );
      const ok = StockLocation(id: 12, name: 'Casa de la moneda');

      expect(
        PrimaryOriginResolver.uniqueOriginKeys([revision, falla, ok]),
        ['Casa de la moneda'],
      );
    });
  });
